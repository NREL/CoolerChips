...
idf_path = os.path.join(
    "1ZoneDataCenterCRAC_wApproachTemp_mod.idf"
)
...
def main():
    ...
    def timestep_callback(state):
        if warmup_done:
            ...
            whole_building_energy = round(api.exchange.get_variable_value(
                state, whole_building_energy_handle
            ), 4)
            print(
    ...
    exit_code = api.runtime.run_energyplus(state, ['-d', output_dir, '-w', epw_path,'-i', 'Energy+.idd', idf_path,])
